...
/*             if (currentPos >= top && currentPos <= bottom) {
    ...
    if (typeof window !== 'undefined') {
        trackSection('home');
        trackSection('subpage1');
        trackSection('subpage2');
        trackSection('subpage3');
    ...
    return (
        <div style={{backgroundColor: `${navBg}`}} className={shadow ? 'fixed w-full h-20 shadow-xl z-[100]' : 'fixed w-full h-20 z-[100]'}> {/* the brackets lets you choose your own value instead of being confined to tailwind's pre set intervals */}
            <div className='flex justify-between items-center w-full h-full px-2 2xl:px-16'>
                <Link href='/'>
                    {/* <Image src="/assets/logo.png" alt="/" width='90' height='40' />  */}
                </Link>
                <div>
                    <ul style={{color: `${linkColor}`}} className='hidden md:flex'> {/* tailwind is mobile first, so this is saying: hide nav items on mobile, and display them as flex on anything above medium screen size */}
                        ...
                        <Link href='/subpage2'>
                            <div id='skillsLink' className='px-5 2xl:px-6 py-2  hover:rounded-xl hover:uppercase hover:bg-gradient-to-r hover:from-[#5651e5] hover:to-[#709dff] hover:text-white'>
                                <li className='text-sm uppercase'>Subpage2</li>
                            </div>
                        </Link>
                        <Link href='/subpage3'>
                            <div id='projectsLink' className='px-5 2xl:px-6 py-2  hover:rounded-xl hover:uppercase hover:bg-gradient-to-r hover:from-[#5651e5] hover:to-[#709dff] hover:text-white'>
                                <li className='text-sm uppercase'>Subpage3</li>
                            </div>
                        </Link>
                    </ul>
